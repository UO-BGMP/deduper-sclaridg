...
parser = argparse.ArgumentParser(description="Removes PCR duplicates from a SAM file of sorted (using samtools) and uniquely mapped reads (single- and paired-end). For each set of duplicates, one duplicate is printed to an output file. Given a list of known UMIs, alignments with unexpected UMIs are ignored. If the reads are paired, assumes reads are mapped in proper pairs")
parser.add_argument('-f','--file', help='absolute path to SAM file to be deduped', required=True, type=str)
parser.add_argument('-p','--paired', help='if set, indicates that reads are paired-end (default: single-end)', required=False, action='store_true', default=False)
parser.add_argument('-u','--umi', help='absolute path to list of UMIs (default: randomers)', required=False, type=str)
parser.add_argument('-q','--quality_filter', help='if set, duplicates will be quality filtered; note that for paired-end reads, quality is based off of forward read only (default: first encountered duplicate)\nperbase: the duplicate with the highest per-base average quality of the set will be printed to the output file (if scores are equal, first encountered read is retained)\nmapq: the duplicate with the highest MAPQ value will be printed to the output file (if MAPQ values are equal, first encountered read is retained)', required=False, action='store_true', default=False)
